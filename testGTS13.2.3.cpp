...
#define GENE_LENGTH 632        // 旋转对称布尔函数的等价类数量
#define FULL_LENGTH 8192       // 13变量布尔函数的全集大小

ofstream process("/Users/mac/Desktop/study/科研/3.18汇报/testGTS/testGTS13.2/改成旋转对称、单点均匀、单点洗牌/process.txt");
ofstream result("/Users/mac/Desktop/study/科研/3.18汇报/testGTS/testGTS13.2/改成旋转对称、单点均匀、单点洗牌/result.txt");
...
void expandToFull8192(int *reduced, int *full, int *representatives) {
    for (int i = 0; i < FULL_LENGTH; i++) { // 遍历 8192 个输入向量
        ...
        for (int j = 0; j < GENE_LENGTH; j++) {
            current = cyclicShiftLeft(current, 13);
...
public:
    ...
    BooleanFunction(int n) : n(n) {
        ...
        do {
    		...
    		expandToFull8192(genes, fullGenes, representatives);
    ...
    BooleanFunction crossover_with_Bent(const BooleanFunction &parent1, const BooleanFunction &parent2) {
	   ...
	   if (rand() % 3 == 0) {
    	...
    	while(ph==false){
        	...
        	// 将 632 位扩展到完整的 8192 位布尔函数
    		expandToFull8192(child.genes, child.fullGenes, child.representatives);
			//cout << "已将 632 位扩展到完整的 8192 位布尔函数\n";
			// 将 int 数组转换为 vector<int>
...
int main() {
	...
	int candidate_list_size=6000;//候选列表长度
	int tabu_size=33;//禁忌列表长度
